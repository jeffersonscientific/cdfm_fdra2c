C     A.M. Bradley (ambrad@cs.stanford.edu) has modified this version of
C     Y. Okada's dc3.f to fix the following problem in DC3D.
C   
C     The essential idea is as follows. DC3D has to compute
C          x11 = 1 / (R + xi), where R = sqrt(xi^2 + eta^2 + q^2),
C     and similar quantities. Along a ray such that (xi < 0 and eta = 0
C     and q = 0), this expression is singular. In the larger context of
C     what DC3D computes, the singularity is nonessential, and so DC3D
C     makes certain substitutions that Okada describes near the end of
C     his 92 paper (see points iii and iv in "Discussion"). Okada also
C     recognized that there can be numerical errors when eta and q are
C     near, but not exactly, 0; again DC3D makes substitutions. The
C     numerical error results from cancellation in R + xi. The issue,
C     however, is that the error must be measured in a relative sense:
C     when
C          sqrt(eta^2 + q^2) / R
C     is small, and not just the numerator, cancellation error
C     occurs. Hence the numerical error occurs in a cone extending into
C     the negative half spaces (xi < 0 or eta < 0) from three of the
C     four corners of the rectangular dislocation. There are four such
C     cones.
C   
C     Simple symmetries in the math problem let us get around the error
C     by using the solution only in the xi, eta > 0 quadrant and
C     flipping signs as necessary to get the solutions in the others.
C
C     The fix is implemented in the routines MQ1 and MQ2. Comment out
C     the calls to these routines to obtain the original behavior.
C
C     ORIGINAL FIX. Nov 2012. A.M. Bradley (ambrad@cs.stanford.edu)
C
C     AMB May 2011. Add THREADPRIVATE OMP statements to the COMMON
C     blocks to make this code threadsafe.
...
C                                                                       
      COMMON /C1/DUMMY1a(8),R,dummy1b(15)
...
C                                                                       
C********************************************************************** 
C*****   AMB WRAPPER TO ORIGINAL DC3D TO PROTECT INPUT VALUES     *****   
C********************************************************************** 
C                                                                       
     ...
     *         UX,UY,UZ,UXX,UYX,UZX,UXY,UYY,UZY,UXZ,UYZ,UZZ
      ...
      CAW1=AW1
      CAW2=AW2
      CALL DC3D1(ALPHA,CX,CY,Z,DEPTH,CDIP,                           
     *     CAL1,CAL2,CAW1,CAW2,CDISL1,CDISL2,DISL3,               
     *     UX,UY,UZ,UXX,UYX,UZX,UXY,UYY,UZY,UXZ,UYZ,UZZ)
      X=CX
      Y=CY
      DIP=CDIP
      ...
      RETURN
      END                                                
      SUBROUTINE  DC3D1(ALPHA,X,Y,Z,DEPTH,DIP,                           
     *                  AL1,AL2,AW1,AW2,DISL1,DISL2,DISL3,               
     *                  UX,UY,UZ,UXX,UYX,UZX,UXY,UYY,UZY,UXZ,UYZ,UZZ)
      IMPLICIT REAL*8 (A-H,O-Z)                                         
      REAL*8   ALPHA,X,Y,Z,DEPTH,DIP,AL1,AL2,AW1,AW2,DISL1,DISL2,DISL3,
...
C*****   DEPTH : SOURCE DEPTH                                           
C*****   DIP   : DIP-ANGLE (DEGREE)                                     
C*****   AL1,AL2   : FAULT LENGTH (-STRIKE,+STRIKE)                     
C*****   AW1,AW2   : FAULT WIDTH  ( DOWNDIP, UPDIP)                     
...
C*****   UXZ,UYZ,UZZ : Z-DERIVATIVE                                     
C                                                                       
      COMMON /C0/DUMMY(5),SD,CD,dummy3(5)
      COMMON /C2/XI2,ET2,Q2,R,dummy2(30)
...
C-----                                                                  
  ...
  111 CONTINUE                                                          
cAMB
      CALL MQ1(FCFS,X,Y,Z,DEPTH,AL1,AL2,AW1,AW2,DIP,DISL1,DISL2)
      AALPHA=ALPHA                                                      
      DDIP=DIP                                                          
      CALL DCCON0(AALPHA,DDIP)                                          
C======================================                                 
...
C-----                                                                  
      ...
      DO 222 J=1,2                                                      
        IF(J.EQ.1) XI=X+AL1                                             
        IF(J.EQ.2) XI=X-AL2                                             
        CALL DCCON2(XI,ET,Q,SD,CD,JXI,JET)                              
        IF(R.EQ.F0)  GO TO 99                                           
...
C=====    
cAMB
      CALL MQ2(FCFS,X,Y,AL1,AL2,AW1,AW2,DIP,DISL1,DISL2,U)
      UX=U(1)                                                           
      UY=U(2)                                                           
      UZ=U(3)                                                           
      UXX=U(4)                                                          
...
cAMB
c   99 UX=F0                                                             
   99 CALL MQ2(FCFS,X,Y,AL1,AL2,AW1,AW2,DIP,DISL1,DISL2,U)
      UX=F0                                                             
      UY=F0                                                             
      UZ=F0                                                             
      UXX=F0                                                            
...
C-----                                                                  
      ...
      RETURN                                                            
      END                                                               
      SUBROUTINE  DCCON2(XI,ET,Q,SD,CD,JXI,JET)                         
      IMPLICIT REAL*8 (A-H,O-Z)                                         
C                                                                       
C********************************************************************** 
...
C*****   JXI,JET : =1 ... IF OBS.POINT IS WITHIN A FAULT RANGE          
C                                                                       
...
C-----                                                                  
      RXI=R+XI                                                          
      IF(JXI.EQ.1 .AND. Q.EQ.F0 .AND. ET.EQ.F0) THEN                    
        ALX=F0                                                          
        X11=F0                                                          
        X32=F0                                                          
      ELSEIF(XI.LT.F0 .AND. Q.EQ.F0 .AND. ET.EQ.F0) THEN                
        ALX=-DLOG(R-XI)                                                 
        X11=F0                                                          
        X32=F0                                                          
      ELSE                                                              
        ALX=DLOG(RXI)                                                   
        X11=F1/(R*RXI)                                                  
        X32=(F2*R+XI)*X11*X11/R                                         
      ENDIF                                                             
C-----                                                                  
      RET=R+ET                                                          
      IF(JET.EQ.1 .AND. Q.EQ.F0 .AND. XI.EQ.F0) THEN                    
        ALE=F0                                                          
        Y11=F0                                                          
        Y32=F0                                                          
      ELSEIF(ET.LT.F0 .AND. Q.EQ.F0 .AND. XI.EQ.F0) THEN                
        ALE=-DLOG(R-ET)                                                 
        Y11=F0                                                          
        Y32=F0                                                          
      ELSE                                                              
...
C-----                                                                  
      ...
      END                                                               
C********************************************************************** 
C*****   AMB ROUTINES TO FIX THE NUMERICAL PROBLEM                ***** 
C********************************************************************** 
      SUBROUTINE MQ1(FCFS,X,Y,Z,DEPTH,AL1,AL2,AW1,AW2,DIP,DISL1,DISL2)
      IMPLICIT NONE
C                                                                       
C********************************************************************** 
C*****   FLIP SIGNS SO (X,Y,Z) IS IN THE XI,ETA > 0 QUADRANT      ***** 
C********************************************************************** 
C                                                                       
      LOGICAL FCFS(2)
      REAL*8 X,Y,Z,DEPTH,AL1,AL2,AW1,AW2,DIP,DISL1,DISL2,ETA,D,CD,SD,
     *     F0,DEG2RAD,TMP
      DATA F0,DEG2RAD/0.D0,1.7453292519943295D-2/
C-----
      SD=DSIN(DIP*DEG2RAD)
      CD=DCOS(DIP*DEG2RAD)
      D=DEPTH+Z
      ...
      RETURN
      END
      SUBROUTINE MQ2(FCFS,X,Y,AL1,AL2,AW1,AW2,DIP,DISL1,DISL2,U)
      IMPLICIT NONE
C                                                                       
C********************************************************************** 
C*****   FLIP SIGNS TO MOVE THE (X,Y,Z) POINT TO ITS QUADRANT     ***** 
C********************************************************************** 
C                                                                       
C***** INPUT                                                            
C*****   FCFS
...
C          
      LOGICAL FCFS(2)
      REAL*8 X,Y,AL1,AL2,AW1,AW2,DIP,DISL1,DISL2,U(12),TMP
C-----
      IF(FCFS(1)) THEN
C     UNDO WHAT WAS DONE IN MQ1
         X=-X
         DISL1=-DISL1
         TMP=AL1
         AL1=AL2
         AL2=TMP
C     FLIP SIGNS TO REFLECT THE SOLUTION BACK INTO THE CORRECT QUADRANT
         U(1)=-U(1)
         U(7)=-U(7)
         U(10)=-U(10)
         U(5)=-U(5)
