...
namespace hmmvp {
  ...
  namespace {
    ...
    template<bool sort_by_col> bool CmpBlockDataCR
    ...
    throw (Exception)
  {
    if (do_perms) {
      ...
      // Do my part.
      Ca::GetTimer()->Tic(catmr_hmat_mvp);
      hm->Mvp(&workr[0], y, ncol);
      Ca::GetTimer()->Toc(catmr_hmat_mvp);

      // Sum all the y's.
      Ca::GetTimer()->Tic(catmr_mpimvp_reduce);
    ...
    Ca::GetTimer()->Toc(catmr_mpimvp_reduce);
  }
